...
- (void) setupAudioCapture {
    ...
    opusInit = opus_ios_enc_init(16000, 1, 16000, 0, 0, 10, 1, initError);
...
#pragma mark -  音频数据(encodedData)
                ...
                int inputByte = 1920;
                int outputByte = 400;
                uint8_t outputBuffer [400];

                uint8 *tempData = (uint8 *)[encodedData bytes];

                int ok = opus_ios_enc_encode(opusInit, tempData, inputByte, (int)[encodedData length], (uint8 *)outputBuffer, &outputByte);
                NSLog(@"%d",ok);
