...
- (void) stopCamera
{
    ...
    int i1 = Livestop_jni(init);
    
    if (i1 != 0) {
        NSLog(@"Stop error! return = %d",i1);
    }
    
    int i2 = Liveclean(init);
...
//                NSLog(@"_tempAudioTimieStamp is %d",_tempAudioTimeStamp);
                ...
                if (SPSPPSSuccess == 1000) {
                    int i = Livepush(init, 1, tempData, (int)[encodedData length], _tempAudioTimeStamp, 0, 0);
                    if (i != 0) {
...
//    NSLog(@"3");
    ...
    SPSPPSSuccess = Livestart(init, (uint8 *)arg_json, (int)[jsonStr lengthOfBytesUsingEncoding:NSUTF8StringEncoding], (uint8 *)[sps bytes], (int)[sps length], (uint8 *)[pps bytes], (int)[pps length]);
    if (SPSPPSSuccess != 0) {
...
//        NSLog(@"_tempVideoTimeStamp is %d",_tempVideoTimeStamp);
        ...
        int i = Livepush(init, 0, tempData, (int)[data length], _tempVideoTimeStamp, isKeyFrame, 0);
        if (i != 0) {
